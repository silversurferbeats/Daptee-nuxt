...
defineProps(['users']);
// console.log('users module -->', users.name)


const showItem = useState('showItem', () => false);
const toggleItem = () => {
...
<template>
    ...
    <div v-if="showItem"
        class="absolute right-12 z-10 mt-28 w-48 origin-top-right rounded-md bg-white py-1 ring-1 shadow-lg ring-black/5 focus:outline-hidden">
        <button class="block px-4 py-2 text-sm text-gray-700">ver</button>
        <button class="block px-4 py-2 text-sm text-gray-700">eliminar</button>
    </div>
</template>
